...
show as list`);
    ...
    if (parsed.ok) {
      const report = validateCellAst(parsed.value, stage0ValidationContext);
      expect(report.errors.some((error) => error.code === 'unknown-field')).toBe(true);
    }
  });
...
show as list`);
    ...
    if (parsed.ok) {
      const report = validateCellAst(parsed.value, stage0ValidationContext);
      expect(report.errors.some((error) => error.code === 'operator-not-allowed')).toBe(true);
    }
  });
...
show as chart`);
  ...
  it('defines usable schemas for every Stage 0 source', () => {
    for (const schema of Object.values(sourceSchemaRegistry)) {
      ...
      expect(Object.values(schema.fields).every((field) => field.allowedOperators.length > 0)).toBe(true);
    }
  });
});
