...
export function parseCanonicalDsl(input: string): Result<CellAst> {
  ...
  if (!sourceMatch) {
    return parseFailure(
      "syntax-source",
      "The first clause must be `from <source>`.",
      lines[0].line,
      1,
    );
  }
  ...
  for (let index = 1; index < lines.length; index += 1) {
    ...
    if (current.text.startsWith("where ")) {
      if (sawShow) {
        return parseFailure(
          "syntax-order",
          "No clauses are allowed after `show as`.",
          current.line,
          1,
        );
      }
      const parsed = parseWhereLine(
        current.text,
        current.line,
        ast.where.length + 1,
      );
    ...
    if (current.text.startsWith("sort by ")) {
      ...
      const match = /^sort by\s+([A-Za-z_][A-Za-z0-9_]*)\s+(asc|desc)$/.exec(
        current.text,
      );
      if (!match) {
        return parseFailure(
          "syntax-sort",
          "Sort clauses must use `sort by <field> asc|desc`.",
          current.line,
          1,
        );
      }
      ast.sort = [
        ...(ast.sort ?? []),
        { field: match[1], direction: match[2] as "asc" | "desc" },
      ];
      continue;
    }
    ...
    if (current.text.startsWith("take ")) {
      ...
      if (sawTake) {
        return parseFailure(
          "syntax-duplicate-take",
          "Only one `take` clause is allowed.",
          current.line,
          1,
        );
      }
      ...
      if (!match) {
        return parseFailure(
          "syntax-take",
          "Take clauses must use `take <positive integer>`.",
          current.line,
          1,
        );
      }
    ...
    if (current.text.startsWith("show as ")) {
      if (sawShow) {
        return parseFailure(
          "syntax-duplicate-renderer",
          "Only one `show as` clause is allowed.",
          current.line,
          1,
        );
      }
      ...
      if (!match || !rendererKinds.includes(match[1] as RendererKind)) {
        return parseFailure(
          "syntax-renderer",
          "Show clauses must use `show as list|count|table|raw`.",
          current.line,
          1,
        );
      }
    ...
    return parseFailure(
      "syntax-unknown",
      `Unsupported clause: ${current.text}`,
      current.line,
      1,
    );
  }
  ...
  if (!sawShow) {
    return parseFailure(
      "syntax-renderer",
      "DSL must end with a `show as <renderer>` clause.",
      lines.at(-1)?.line ?? 1,
      1,
    );
  }
...
function parseWhereLine(
  text: string,
  line: number,
  index: number,
): Result<CellAst["where"][number]> {
  ...
  if ((op === "exists" || op === "not_exists") && rawValue !== undefined) {
    return parseFailure(
      "syntax-where",
      "`exists` predicates do not take a value.",
      line,
      text.indexOf(rawValue) + 1,
    );
  }
  if (op !== "exists" && op !== "not_exists" && rawValue === undefined) {
    return parseFailure(
      "syntax-where",
      "This predicate requires a value.",
      line,
      text.length,
    );
  }
...
function parseValue(
  raw: string,
  line: number,
): Result<LiteralValue | FunctionCallValue> {
  const trimmed = raw.trim();
  if (/^"(?:[^"\\]|\\.)*"$/.test(trimmed)) {
    try {
      return {
        ok: true,
        value: { kind: "string", value: JSON.parse(trimmed) as string },
      };
    } catch {
      return parseFailure(
        "syntax-string",
  ...
  if (noArgFunction) {
    return {
      ...
      value: {
        kind: "function",
        name: noArgFunction[1] as FunctionCallValue["name"],
        args: [],
      },
    };
  }
  ...
  return parseFailure(
    "syntax-value",
    `Unsupported value literal: ${trimmed}`,
    line,
    1,
  );
}
