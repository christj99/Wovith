...
export function buildEvidenceForItem(input: {
  ...
  const { ast, cell, evaluationId, item, nowIso, snapshotId, sourceSchema } =
    input;
  ...
  return {
    ...
    sortEvidence: sort
      ...
      : undefined,
    selectedFields: Object.values(sourceSchema.fields).map<FieldEvidence>(
      (field) => {
        ...
        return {
          ...
          redactedPreview: field.sensitive
            ? "[redacted]"
            : previewValue(tainted?.value),
...
export function predicateEvidenceLabel(predicate: PredicateEvidence): string {
  const value = predicate.expected
    ? ` ${previewAstValue(predicate.expected)}`
    : "";
...
function sourceTimestamp(
  item: SourceItem,
): ProvenanceEvidence["sourceTimestamp"] {
  const candidates = [
    "updated_at",
    "modified_at",
    "received_at",
...
function previewItem(item: SourceItem, sourceSchema: SourceSchema): string {
  const titleField = ["subject", "title", "name"].find(
    (field) => field in sourceSchema.fields,
  );
